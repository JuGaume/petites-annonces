...
public static class DigestEmailTemplate
{
    ...
    public static string RenderSavedSearchAlert(SavedSearchAlert alert, string groupUrl)
    {
        var items = new StringBuilder();
        foreach (var listing in alert.Matches)
        {
            var price = listing.Price is not null ? $" — {listing.Price} €" : string.Empty;
            items.Append($"<li>{WebUtility.HtmlEncode(listing.Title)}{price}</li>");
        }
        ...
        return $"""
            <p>Bonjour {WebUtility.HtmlEncode(alert.UserDisplayName)},</p>
            <p>{alert.Matches.Count} nouvelle(s) annonce(s) correspondent à votre alerte
            <strong>« {WebUtility.HtmlEncode(alert.Label)} »</strong> dans le groupe
            <strong>{WebUtility.HtmlEncode(alert.GroupName)}</strong> :</p>
            <ul>{items}</ul>
            <p><a href="{groupUrl}">Voir les annonces du groupe</a></p>
            <p style="color:#6b7280;font-size:12px">Vous pouvez supprimer cette alerte depuis la page des annonces du groupe.</p>
            """;
    }
}
